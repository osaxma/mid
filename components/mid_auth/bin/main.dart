...
void main() async {
  final authDB = AuthSqlite(dbPath: '/Users/osaxma/Projects/mid/components/mid_auth/bin/test.db');
  final jwtHandler = JWTHandlerRsa256(
    jwtPrivateKey: samplePrivateKey,
    jwtPublicKey: samplePublicKey,
    aud: 'aud',
    issuer: 'issuer',
  );
...
Future<void> signInThenRefreshToken(Auth auth) async {
  ...
  final newSession = await auth.refreshSession(session.accessToken, session.refreshToken);
...
Future<void> signInSignOutThenTryRefreshToken(Auth auth) async {
  ...
  await auth.signOut(session.accessToken, session.refreshToken);
